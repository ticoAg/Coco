...
enum CliError {
    #[error("{0}")]
    Store(#[from] agentmesh_core::task_store::TaskStoreError),
    #[error("{0}")]
...
fn cmd_subagent_spawn(
    ...
    let resp = orchestrator.subagent_spawn(agentmesh_orchestrator::SubagentSpawnRequest {
...
fn resolve_workspace_root() -> Result<PathBuf, CliError> {
    ...
    let project_dirs =
        ProjectDirs::from("dev", "agentmesh", "agentmesh").ok_or(CliError::AppDataUnavailable)?;
...
fn exit_code_for_error(err: &CliError) -> u8 {
    match err {
        CliError::Store(agentmesh_core::task_store::TaskStoreError::TaskNotFound { .. }) => {
            EXIT_CODE_NOT_FOUND
        }
...
fn format_task_state(state: agentmesh_core::task::TaskState) -> &'static str {
    match state {
        agentmesh_core::task::TaskState::Created => "created",
        agentmesh_core::task::TaskState::Working => "working",
        agentmesh_core::task::TaskState::InputRequired => "input-required",
        agentmesh_core::task::TaskState::Completed => "completed",
        agentmesh_core::task::TaskState::Failed => "failed",
        agentmesh_core::task::TaskState::Canceled => "canceled",
    }
}
