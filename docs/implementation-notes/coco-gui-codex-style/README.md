# Coco GUI × Codex VSCode 插件 UI/UX 复刻：样式映射与组件骨架

本目录用于把「Coco GUI」在 UI/UX 上向「Codex VSCode 插件 Webview」靠齐的实现方案沉淀下来，便于后续同事接力扩展。
...
- 希望 Coco GUI 的页面内容整体参考复刻 VSCode 插件：Codex 的 UI/UX 样式（暗色材质、层级、密度、卡片、折叠交互、hover 反馈）。
- 本期只做：`CodexChat` 页面。
