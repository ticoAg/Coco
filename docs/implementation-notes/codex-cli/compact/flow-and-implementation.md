...
> 目标：沉淀 Codex CLI（`codex/codex-rs`）里 `/compact` 的端到端实现路径，精确到关键文件与行号，便于 AgentMesh GUI 复刻同等行为。
>
...
- 枚举定义：`codex/codex-rs/tui2/src/slash_command.rs:12`
- 说明文案：`codex/codex-rs/tui2/src/slash_command.rs:45`
- “任务运行时不可用”的 gating：`codex/codex-rs/tui2/src/slash_command.rs:71`
...
- `codex/codex-rs/protocol/src/protocol.rs:205`（注释说明）
- `codex/codex-rs/protocol/src/protocol.rs:208`（`Compact,` 枚举值）
...
- handler：`codex/codex-rs/core/src/codex.rs:2086`
  - `turn_context.compact_prompt()` 用于获取 prompt
  - `spawn_task(..., CompactTask)` 启动任务
- prompt 选择逻辑：`codex/codex-rs/core/src/codex.rs:400`
...
- `codex/codex-rs/core/src/codex.rs:400`

## 4. CompactTask：remote vs local 的选择逻辑（核心分支点）

`CompactTask` 是 `/compact` 的实际执行任务。它会根据 `should_use_remote_compact_task(...)` 选择远端或本地路径：

- 任务入口：`codex/codex-rs/core/src/tasks/compact.rs:20`
- 分支判断：`codex/codex-rs/core/src/tasks/compact.rs:28`
...
1) `provider.is_openai()` 只检查 provider 的 **display name** 是否等于 `"OpenAI"`：
   - 常量：`codex/codex-rs/core/src/model_provider_info.rs:31`
   - 判定：`codex/codex-rs/core/src/model_provider_info.rs:253`
2) `Feature::RemoteCompaction` 的默认值在本 repo 中是 `default_enabled: true`：
   - 枚举定义注释写明 “ChatGPT auth only”：`codex/codex-rs/core/src/features.rs:83`
   - 默认开启：`codex/codex-rs/core/src/features.rs:388`
...
- 发送 `TurnStarted`：`codex/codex-rs/core/src/compact.rs:52`
- 真正执行：`codex/codex-rs/core/src/compact.rs:64`

在 `run_compact_task_inner()` 中，它用 `history.clone().for_prompt()` 作为 input，构建一个 `Prompt` 并发起 streaming：

- `Prompt { input: turn_input, ..Default::default() }`：`codex/codex-rs/core/src/compact.rs:101`
- drain loop：`codex/codex-rs/core/src/compact.rs:295`
  - 每个 `ResponseEvent::OutputItemDone` 会被写入 history：`codex/codex-rs/core/src/compact.rs:310`
  - 收到 `Completed` 后返回：`codex/codex-rs/core/src/compact.rs:317`

### 5.3 ContextWindowExceeded 时的处理（裁剪旧历史）

如果模型返回 “context window exceeded”，本地路径会从历史最前端开始丢弃 item，并重试：

- 分支入口：`codex/codex-rs/core/src/compact.rs:97`
- 处理 `CodexErr::ContextWindowExceeded`：`codex/codex-rs/core/src/compact.rs:123`

这里的策略是“尽量保留最近消息”：

- Trim from beginning（移除最老的 history item）：`codex/codex-rs/core/src/compact.rs:125`
  - 实际删除：`history.remove_first_item()`：`codex/codex-rs/core/src/compact.rs:129`

### 5.4 summary_text 是怎么生成的（关键：summary 存为 user message）

本地总结完成后，core 取 “本次 compact turn 的最后一条 assistant message” 作为 summary 内容：

- 取 summary：`codex/codex-rs/core/src/compact.rs:162`
  - `get_last_assistant_message_from_turn(history_items)`
- 拼 summary_text：`codex/codex-rs/core/src/compact.rs:163`
  - `format!("{SUMMARY_PREFIX}\n{summary_suffix}")`

接着会调用 `build_compacted_history(...)` 重建历史：

- `build_compacted_history`：`codex/codex-rs/core/src/compact.rs:231`

这里有一个非常重要的实现细节：

- summary 被写成 `ResponseItem::Message { role: "user", content: InputText(summary_text) }`
  - 具体写入点：`codex/codex-rs/core/src/compact.rs:286`
...
- `collect_user_messages`：`codex/codex-rs/core/src/compact.rs:211`
- 它会跳过 summary message（通过 `is_summary_message` 判断）：`codex/codex-rs/core/src/compact.rs:227`

此外，哪些东西算 “user message” 由 `crate::event_mapping::parse_turn_item` 决定，它会过滤掉：

- `<environment_context>...` 这类 session prefix：`codex/codex-rs/core/src/event_mapping.rs:24`
- `AGENTS.md instructions` / `skills` 注入：`codex/codex-rs/core/src/event_mapping.rs:31`
- 以及 user 直接发起的 `!shell`（避免被当作“自然语言上下文”带入）：`codex/codex-rs/core/src/event_mapping.rs:50`

对应实现位置：

- `parse_user_message(...)`：`codex/codex-rs/core/src/event_mapping.rs:30`
- session prefix 判定：`codex/codex-rs/core/src/event_mapping.rs:24`
...
- 上限常量：`codex/codex-rs/core/src/compact.rs:33`
- 算 token 并选择：`codex/codex-rs/core/src/compact.rs:250`
- 超限时会对最老的一条做截断后停止：`codex/codex-rs/core/src/compact.rs:262`
...
- `replace_history`：`codex/codex-rs/core/src/compact.rs:174`
- ghost snapshots（保留 `/undo`）：`codex/codex-rs/core/src/compact.rs:168`
- rollout：`codex/codex-rs/core/src/compact.rs:177`
- `ContextCompacted`：`codex/codex-rs/core/src/compact.rs:183`
- warning：`codex/codex-rs/core/src/compact.rs:186`
...
- 构造 Prompt：`codex/codex-rs/core/src/compact_remote.rs:53`
  - `input: history.for_prompt()`
  - `base_instructions_override: turn_context.base_instructions.clone()`
  - tools 为空：`tools: vec![]`
- 发起请求：`codex/codex-rs/core/src/compact_remote.rs:61`
...
- `codex/codex-rs/core/src/client.rs:354`
...
- `prompt.get_full_instructions(&self.get_model_info())`：`codex/codex-rs/core/src/client.rs:372`
  - 即：由“当前模型的 base instructions / override”决定

endpoint 路径来自 `codex-api` 的实现：

- `codex/codex-rs/codex-api/src/endpoint/compact.rs:39`
...
- 合并 ghost snapshots：`codex/codex-rs/core/src/compact_remote.rs:45`
- replace：`codex/codex-rs/core/src/compact_remote.rs:69`
- rollout：`codex/codex-rs/core/src/compact_remote.rs:72`
- `ContextCompacted`：`codex/codex-rs/core/src/compact_remote.rs:79`

与本地总结不同点：

- remote 路径不会发送 “Heads up...” warning（`compact_remote.rs` 中没有对应事件）。
- remote 路径的 error 处理是“转成 `ErrorEvent` 并结束”，不会 fallback：
  - `codex/codex-rs/core/src/compact_remote.rs:30`
...
- `codex/codex-rs/core/src/model_provider_info.rs:130`
  - 选择 base_url：`codex/codex-rs/core/src/model_provider_info.rs:134`
...
- `codex/codex-rs/tui2/src/chatwidget.rs:1990`
...
- Turn started（开始执行 compact task）：`codex/codex-rs/tui2/src/chatwidget.rs:1913`
- Turn complete（compact 任务结束）：`codex/codex-rs/tui2/src/chatwidget.rs:1914`
- Context compacted（历史已被替换）：`codex/codex-rs/tui2/src/chatwidget.rs:1990`
- Warning / Error（本地会额外 warning；远端失败可能 error）：`codex/codex-rs/tui2/src/chatwidget.rs:1921`
...
- 本地 auto-compact 入口：`codex/codex-rs/core/src/compact.rs:42`（`run_inline_auto_compact_task`）
- 远端 auto-compact 入口：`codex/codex-rs/core/src/compact_remote.rs:14`（`run_inline_remote_auto_compact_task`）
