...
- **可复盘**：每次任务都有唯一事实来源：`.coco/tasks/<task_id>/`
...
- **Task Directory（产物面）**
  ...
  - 目录规范见：[`docs/coco/artifacts.md`](./artifacts.md)
- **Controller（程序状态机）**
...
1) [`docs/coco/artifacts.md`](./artifacts.md)：Task Directory 与产物规范（事实来源）
2) [`docs/coco/subagents.md`](./subagents.md)：如何用 `codex exec --json` 跑并行 workers
3) [`docs/coco/adapters/codex.md`](./adapters/codex.md)：Codex adapter（exec / app-server）接入要点
4) [`docs/coco/gui.md`](./gui.md)：GUI（Artifacts-first + 可选 Codex Chat）
