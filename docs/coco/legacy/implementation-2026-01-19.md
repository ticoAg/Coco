...
> 注意：本文为 2026-01-19 从 [`docs/coco/implementation.md`](../implementation.md) 迁移的历史备份，可能与当前主线文档不一致。
>
> 当前主线入口：
> - 执行闭环：[`docs/coco/execution.md`](../execution.md)
> - 实现评估（主线）：[`docs/coco/implementation.md`](../implementation.md)

> 目标：参考 [`docs/references/`](../../references) 下的文档，评估如何落地 [[Coco.md]](../../Coco.md) 的设计，并把**可让用户随时介入与修正**的产物形态作为首要产出。
>
> 重要前提：本项目核心是**直接复用各方成熟的 code TUI/CLI 产品**（例如 codex cli 这类交互式命令行）。A2A / ACP / Claude Code Subagents 的概念只用于借鉴交互模型与术语，不作为硬依赖或必做集成。

> 当时的 multi/subagent 整合稿见：[`docs/coco/legacy/multiagent-2026-01-19.md`](./multiagent-2026-01-19.md)。
>
> 术语约定（避免歧义）：
> - **Orchestrator（模型）**：负责规划/分解/验收，输出结构化 actions。
> - **Controller（程序）**：负责状态机执行/并发调度/落盘/证据链/权限隔离（仓库代码里通常落在 `coco-orchestrator` crate）。

## 1. 设计要点回顾（从 Coco 设计抽象成工程需求）

从 [[Coco.md]](../../Coco.md) 与 [[README.md]](../../README.md) 可抽象出以下“必须能落地”的能力：
...
**但**本项目不计划直接复用 Claude Code 的 Subagents 技术实现（例如 `.claude/agents/*.md`、`/agents`、`resume agentId` 等）。在 Coco 中：
...
在 Coco 中，Skills 的职责边界应保持克制：

- Coco 负责：**skills 资产的管理与分发**、以及“给哪个 agent 预装哪些 skill sets”的配置
...
在 Coco 中，这些概念可作为**术语与建模参考**：

- `Task` → `.coco/tasks/<task_id>/`
...
在 Coco 中，ACP 仅作为参考：本项目先把“CLI 工具 session 化 + 结构化产物 + 人工介入”打通，是否提供 ACP 兼容层属于后置增强，而不是核心路径。
...
对 Coco 来说，它非常适合作为 **子代理（subagents）并发执行** 的 MVP：
...
细节见：[`docs/coco/subagents.md`](./subagents.md) 与 [`docs/coco/adapters/codex.md`](./adapters/codex.md)。

Codex adapter 的具体交互与落盘细节见：[`docs/coco/adapters/codex.md`](./adapters/codex.md)。

## 4. 总体架构（Session 驱动 + 产物驱动）

不做多 TUI 控制台时，Coco 实际管理的是“一组 coder sessions”，核心是：**如何创建/恢复 session、如何跑一轮 turn、如何把事件流写入任务目录并生成产物**。
...
- **Adapter（Codex-first）**
  ...
  - 把 Codex 的事件流落盘（JSONL），并提取关键内容生成 Coco 产物
- **产物面（Task Directory）**
...
- **审批对齐**：把 Codex 的审批请求映射为 Coco 的 `gate.blocked`，等待用户决策后再回传 allow/deny
...
- **Codex 事件到 Coco 产物的映射**：哪些事件写 `events.jsonl`，哪些提取为 `reports/*.md`
