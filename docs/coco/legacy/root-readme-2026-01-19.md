...
>
> 当前入口建议从这里开始：
> - 项目导航：[`Coco.md`](../../../Coco.md)
> - 落地文档索引：[`docs/coco/README.md`](../README.md)

Coco 是一个多 `Code Agent` 编排框架，用于协调多个异构 agent（如 `codex`、`Claude Code`、`gemini-cli`、`qwen-code` 等）高效完成复杂开发任务。
...
从这里开始（项目导航页）：[`Coco.md`](../../../Coco.md)

文档总索引：[`docs/README.md`](../../README.md)

实现评估与多阶段路线图见：[`docs/coco/README.md`](../README.md)

Codex adapter 说明见：[`docs/coco/adapters/codex.md`](../adapters/codex.md)

执行闭环总览见：[`docs/coco/execution.md`](../execution.md)

## 目录约定（示例）

- `.coco/agents/<agent_name>/agents.md`：该 agent 的角色定义（职责、输入/输出、权限、可见范围等），可由 Lead 生成或使用预置模板
- `.coco/agents/<agent_name>/skills/<skill_name>/SKILL.md`：该 agent 的 skill（一个 skill 是包含 `SKILL.md` 的自包含文件夹，可附带脚本/资源）
- `.coco/tasks/<task_id>/README.md`：该任务的入口（目标、状态、里程碑、关键链接）
- `.coco/tasks/<task_id>/shared/**`：任务级共享资产（例如契约、决策、汇总报告等）
- `.coco/tasks/<task_id>/agents/<agent_instance>/README.md`：agent 实例（coder session）在本任务内的产出索引（沉淀/摘要/链接）
- `.coco/tasks/<task_id>/agents/<agent_instance>/**/*.md`：该 agent 实例在任务执行/代码探索中产出的可共享内容（都需要元数据）
...
在 Coco 的语境里，Context Sharing 更像一种内部工作机制：我们把一次协作任务落盘为可持久化的 `Task Directory`；每个 agent 实例（coder session）把产出沉淀在自己的子目录里，需要共享时再通过“显式附加（explicit attach）”引用相关文件/片段，避免全量上下文广播。
...
flowchart LR
  ...
  A[Agent] --> TD["Task Dir<br/>.coco/tasks/&lt;task_id&gt;/"]
