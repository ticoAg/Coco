...
> 注意：本文为 2026-01-19 从 [`docs/coco/roadmap.md`](../roadmap.md) 迁移的历史备份，可能与当前主线文档不一致。
>
> 当前主线入口：[`docs/coco/roadmap.md`](../roadmap.md)（以及 [`docs/coco/execution.md`](../execution.md)）。

> 原则：先把“产物形态 + 人工介入点”做扎实；执行层以 **CLI 工具的底层可编程接口** 作为首个接入路径（先做 Codex），直接读取结构化输出。
>
> A2A / ACP / Claude Code Subagents 仅用于参考概念，不作为本项目核心依赖。

> 当时的 multi/subagent 执行方案整合稿见：[`docs/coco/legacy/multiagent-2026-01-19.md`](./multiagent-2026-01-19.md)。
>
...
- skills 定义遵循 [`docs/references/skills/README.md`](../../references/skills/README.md)，不做协议扩展

**交付物**
- skills 仓库与选择机制（例如 `.coco/agents/<agent>/skills/<skill>/SKILL.md`）
