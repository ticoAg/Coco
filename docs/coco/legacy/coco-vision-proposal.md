# (Archived) Coco Vision Proposal

> 注意：本文为早期愿景/草案归档，可能包含过时表述或与当前实现方向不完全一致。
>
> 建议优先从以下入口开始：
> - 项目导航页：[`Coco.md`](../../../Coco.md)
> - 落地文档索引：[`docs/coco/README.md`](../README.md)
...
>
> - 把成熟 CLI 工具（先做 Codex）当作“coder runtime”，后台维护其 session（例如 Codex 的 `threadId`），并通过底层事件流直接提取结构化输出（`codex app-server` / `codex exec --json`），落盘为 `.coco/tasks/<task_id>/...` 的可介入产物。
> - 这里提到的 “Subagent” 更偏向“角色/分工/阵容”的概念，不绑定某家产品的 Subagents 技术实现，也不要求做多 TUI 控制台。
>
> 具体落地文档见：[`docs/coco/README.md`](../README.md)。
> - 执行闭环总览（Task Directory + Workers + Gates / Evidence-first）见：[`docs/coco/execution.md`](../execution.md)。
> - Codex adapter 说明：[`docs/coco/adapters/codex.md`](../adapters/codex.md)。
...
- **协作逻辑**：
  ...
  - **Agent 定义生成 (Agent Spec Generation)**：为每个角色生成一份可复用的 `agents.md`（例如 `.coco/agents/db/agents.md`），作为该 agent 的“说明书/运行契约”（Role、Inputs/Outputs、Permissions、Scope、Triggers 等）。
    - 仓库里也可以保留一份模板库（例如 [`agents/db/agents.md`](../../../agents/db/agents.md)），供复制/微调后写入 `.coco/agents/`。
...
在 Coco 的语境里，“Context Sharing”更偏向一种内部工作机制：一次完整的 `User` - `Agents` 交互通常会被组织为一个 `Task`（任务），并落盘为可持久化的任务目录；任务内的 agent 实例（coder session）可以在自己的子目录中沉淀探索过程与结果。需要共享时，通过“显式附加 (explicit attach)”引用必要内容，避免全量上下文广播。

- **Task Directory**：每次任务交互对应一个目录，例如：
  - `.coco/tasks/<task_id>/README.md`（必需）：任务入口（目标、状态、里程碑、关键链接）
  - `.coco/tasks/<task_id>/shared/**`：任务级共享资产（契约、决策、汇总报告等）
  - `.coco/tasks/<task_id>/agents/<agent_instance>/README.md`（常见）：该 agent 实例在本任务内的产出索引（摘要、链接）
  - `.coco/tasks/<task_id>/agents/<agent_instance>/artifacts/**`：探索笔记、决策记录、排查日志、接口契约等（可选，常见）
  - `.coco/tasks/<task_id>/agents/<agent_instance>/runtime/**`：工具侧事件流与 rollout（例如 Codex `events.jsonl` / `rollout.jsonl`，用于复盘与提取）

同时，agent 的“行为定义/说明书”应独立存放为可复用模板：

- **Agent Spec (`agents.md`)**：系统为每个 agent 角色生成一份可复用的 `agents.md`（例如 `.coco/agents/db/agents.md`），指导该角色/会话的行为（Role、Inputs/Outputs、Permissions、Scope、Triggers 等）。
...
Coco 把 Skills 视为各家 agent 可共享的一种“能力封装形式”（指令、脚本、资源）。Skills 的定义以 [[skills/README.md]](docs/references/skills/README.md) 为准；不同产品（例如 codex）如何加载/呈现 Skills 属于其运行时实现细节（可参考 [[openai-codex/skills.md]](docs/references/openai-codex/skills.md)），不应反向成为 Coco 的 Skills 规范扩展。
...
- **归档方式（按 agent）**：`.coco/agents/<agent_name>/skills/<skill_name>/SKILL.md`（一个 skill 一个文件夹）。
