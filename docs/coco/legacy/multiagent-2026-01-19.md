...
> 注意：本文为 2026-01-19 从 `docs/coco/multiagent.md` 迁移的历史备份，可能与当前主线文档不一致。
>
> 当前主线入口：
> - 执行闭环：[`docs/coco/execution.md`](../execution.md)
> - 落地文档索引：[`docs/coco/README.md`](../README.md)

> 本文是对 Coco 的 multi-agent / subagent 方案的一份“可落地”整合稿：
...
在 Coco 中，`app-server` 既可以服务 GUI，也可以作为 Controller 的 adapter（建议抽成可复用库）。
...
- 新增 `codex-app-server-adapter` capability：把 app-server 的会话/事件/审批能力纳入 Coco adapter 层，并落盘。
