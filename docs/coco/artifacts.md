...
> 本仓库当前的 `agents/*/agents.md` 可视为“模板库”，未来可复制/同步到 `.coco/agents/`。

## 2. Task Directory（任务目录）

每次任务交互生成一个目录：`.coco/tasks/<task_id>/`。
...
API Contract 通常落在 `.coco/tasks/<task_id>/shared/contracts/` 下，保持“契约为中心”的协作方式。
