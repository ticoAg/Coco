...
## Purpose
Coco 是一个“产物优先（artifacts-first）”的本地编排系统：把一次人类与多个 coding agents 的协作过程，落盘为可追踪、可编辑、可复现的任务目录（`.coco/tasks/*`）。

当前路线选择 **Codex-first**：优先通过 `codex exec --json` / `codex app-server` 这类“底层可编程接口”消费结构化事件流，而非解析 TUI/ANSI 屏幕。

## Tech Stack
- Rust workspace（[`crates/coco-core`](../crates/coco-core), [`crates/coco-orchestrator`](../crates/coco-orchestrator), [`crates/coco-codex`](../crates/coco-codex)）
...
### Code Style
- 倾向小而清晰的实现；避免过度抽象。
- 尽量复用现有模式（例如 `coco-core` 的 `TaskStore` 负责落盘读写）。
...
- 避免在 worker 内执行 `git merge/rebase/push`（参见 [`docs/coco/prompts/codex-worker.md`](../docs/coco/prompts/codex-worker.md)）。

## Domain Context
- `.coco/tasks/<task_id>/` 是单个任务的落盘空间。
...
- 外部运行时：`codex` 可执行文件需要在用户机器 PATH 中可用（Coco 不负责打包/分发）。
