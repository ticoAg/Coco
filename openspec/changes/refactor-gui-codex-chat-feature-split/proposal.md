...
- `apps/gui/src/components/CodexChat.tsx`（约 5k+ 行）
...
## What Changes
- 引入 `apps/gui/src/features/codex-chat/` 作为 Codex Chat 的唯一业务域入口，并提供 Facade：`apps/gui/src/features/codex-chat/index.ts`。
...
- 不改变 Codex Chat 的 UI/UX、交互细节与 VSCode plugin parity（以 `openspec/specs/gui-codex-chat/spec.md` 与 `docs/implementation-notes/agentmesh-gui-codex-style/README.md` 为准）。
...
- Likely affected code (implementation stage):
  ...
  - `apps/gui/src/hooks/useSessionTree.ts`（可能迁移到 feature）
  - `apps/gui/src/types/codex.ts`（可能迁移到 feature 或 shared 并通过 Facade 暴露）
- Likely affected docs:
  - `docs/implementation-notes/agentmesh-gui-codex-style/README.md`（存在硬编码路径引用）
