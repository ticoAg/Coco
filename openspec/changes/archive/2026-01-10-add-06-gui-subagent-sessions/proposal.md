...
## Why
当前 GUI 已能列出任务与任务级事件，但尚不能展示 per-agent runtime/events 与最终结构化输出；而 `docs/agentmesh/gui.md` 明确把 sessions 视为核心信息架构的一部分。
...
- 受影响代码（实现阶段）：`apps/gui`（React UI）、`apps/gui/src-tauri`（如需新增读取接口/轮询）
