...
## 2. Implementation
- [x] 2.1 后端：新增 thread title sidecar 读写（`.coco/codex/threads/<thread_id>.json`）。
- [x] 2.2 后端：`codex_thread_list` 注入 `title`（sidecar 优先；缺失则按规则生成 50 字并落盘）。
...
- [x] 2.6 前端：session tree 增加 Active/Archived + 日期/小时分组，并在分组节点提供一键归档入口（完成后刷新列表）。
- [x] 2.7 前端：若 3min 内存在更新的会话，则开启 3min 的 10s 轮询窗口。
