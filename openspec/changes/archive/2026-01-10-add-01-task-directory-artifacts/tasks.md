...
## 1. Spec
- [x] 将 `docs/agentmesh/artifacts.md` 的 Task Directory 关键约定固化到 `task-directory` 规格（必需文件、目录、最小字段、兼容策略）。
...
- [x] 必要时更新 `schemas/task.schema.json`（与 Rust `TaskFile` / GUI types 一致）。
- [x] 更新 `docs/README.md`：增加一条指向 `openspec/` 的索引入口（说明这里是 specs/changes 的位置）。
