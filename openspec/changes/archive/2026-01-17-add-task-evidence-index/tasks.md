...
## 2. Implementation
- [x] 2.1 `coco-core`: 创建任务时 scaffold `shared/evidence/` 与 `shared/evidence/index.json`（空数组）。
- [x] 2.2 `coco-orchestrator`: 在 join/report 生成阶段写入 evidence index，并在 `joined-summary.md` 中引用。
...
- [x] 3.2 `cargo test -p coco-core -p coco-orchestrator`
