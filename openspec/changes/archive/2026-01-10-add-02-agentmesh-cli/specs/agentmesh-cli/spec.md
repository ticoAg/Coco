...
- `agentmesh task events <task_id>`

#### Scenario: Task commands are discoverable
- **WHEN** 用户执行 `agentmesh task --help`
...
- **WHEN** 执行 `agentmesh task show <task_id> --json`
- **THEN** 输出为 JSON 对象

#### Scenario: Create task outputs id
- **WHEN** 执行 `agentmesh task create --title "X" --topology swarm --json`
...
- **WHEN** 执行 `agentmesh task show <missing_task_id>`
- **THEN** CLI 以退出码 3 结束
...
- **GIVEN** 任务存在
- **WHEN** 执行 `agentmesh task show <task_id>`
