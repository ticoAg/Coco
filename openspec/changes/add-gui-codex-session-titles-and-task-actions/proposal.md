...
## Summary
在 GUI 的 Codex Chat（session tree）中补齐两个核心体验：

- **会话标题（title）**：当 codex app-server 只返回 `preview`（首条用户消息摘要）时，GUI 侧按规则提取生成一个短标题（自动：25 字），并把结果持久化到工作区目录（`.agentmesh/`）以便下次打开仍可复用；支持手动改名（最多 50 字）。
...
## Data / Storage
- 自动/手动标题 sidecar（per-workspace）：
  - 路径：`<workspace_root>/.agentmesh/codex/threads/<thread_id>.json`
  - 字段：`title`（string）、`source`（`generated-v1`/`manual`）、`updatedAtMs`（number|null）
