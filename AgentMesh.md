...
>
> 具体落地文档见：[`docs/agentmesh/README.md`](./docs/agentmesh/README.md) 与 Codex adapter 说明：[`docs/agentmesh/adapters/codex.md`](./docs/agentmesh/adapters/codex.md)。
...
- **Task Directory**：每次任务交互对应一个目录，例如：
  ...
  - `.agentmesh/tasks/<task_id>/agents/<agent_instance>/README.md`（推荐）：该 agent 实例在本任务内的产出索引（摘要、链接）
  - `.agentmesh/tasks/<task_id>/agents/<agent_instance>/artifacts/**`：探索笔记、决策记录、排查日志、接口契约等（可选，但推荐）
...
- **建议的归档方式（按 agent）**：`.agentmesh/agents/<agent_name>/skills/<skill_name>/SKILL.md`（一个 skill 一个文件夹）。
